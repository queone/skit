...
for flag in -h '-?' --help; do
  ...
  grep -q '^dos2unix v0\.1\.4$' "$fixture/out" || fail "dos2unix $flag version header"
  ...
  grep -q '^tree v0\.1\.4$' "$fixture/out" || fail "tree $flag version header"
...
for flag in -v --version; do
  [ "$(NO_COLOR=1 "$dos2unix_bin" ignored "$flag" extra)" = 'dos2unix v0.1.4' ] ||
    fail "dos2unix $flag output"
  [ "$(NO_COLOR=1 "$tree_bin" ignored "$flag" extra)" = 'tree v0.1.4' ] ||
